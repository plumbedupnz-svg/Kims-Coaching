-- Add minimum age and minimum level rules to reusable lesson types.
...
alter table public.lesson_types
drop constraint if exists lesson_types_minimum_age_valid;

alter table public.lesson_types
add constraint lesson_types_minimum_age_valid
check (minimum_age is null or (minimum_age >= 0 and minimum_age <= 120));

alter table public.lesson_types
drop constraint if exists lesson_types_minimum_level_valid;

alter table public.lesson_types
add constraint lesson_types_minimum_level_valid
check (coalesce(minimum_level, '') in ('', 'Beginner', 'Developing', 'Interclub', 'Tournament'));
