-- Add per-slot minimum player requirements for coaching availability.

alter table public.availability
add column if not exists minimum_players integer not null default 1;

alter table public.availability
drop constraint if exists availability_minimum_players_positive;

alter table public.availability
add constraint availability_minimum_players_positive
check (minimum_players >= 1);

alter table public.availability
drop constraint if exists availability_minimum_players_not_above_capacity;

alter table public.availability
add constraint availability_minimum_players_not_above_capacity
check (minimum_players <= capacity);
...
as $$
  with private_lesson as (
    select id, name, duration, price, capacity, minimum_players, pay_as_you_go_only
