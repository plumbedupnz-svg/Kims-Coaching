...
begin
  if not exists (
    ...
    check (payment_option in ('pay_now', 'pay_later'));
  end if;
...
alter table public.bookings
drop constraint if exists bookings_bundle_id_fkey;

alter table public.bookings
add constraint bookings_bundle_id_fkey
foreign key (bundle_id) references public.lesson_bundles(id) on delete set null;
...
grant select on public.lesson_bundles to anon, authenticated;
grant insert, update, delete on public.lesson_types to authenticated;
grant insert, update, delete on public.lesson_bundles to authenticated;

create index if not exists availability_lesson_type_idx on public.availability(lesson_type_id);
create index if not exists bookings_availability_time_idx on public.bookings(availability_id, start_time, end_time);
create index if not exists lesson_bundles_lesson_type_idx on public.lesson_bundles(lesson_type_id);
...
as $$
  ...
  expanded_slots as (
    select
      ...
      coalesce(lesson_types.name, private_lesson.name, 'Private Lesson') as lesson_type_name,
...
begin
  if auth.uid() is null then
    raise exception 'You must be logged in to book a lesson.';
  end if;
...
grant execute on function public.create_private_lesson_booking(uuid, timestamptz, uuid, integer, text, text, text, text, text, text, text, text, uuid, integer, numeric, numeric) to authenticated;
