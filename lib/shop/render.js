...
<link rel="stylesheet" href="/styles.css?v=20260922"><link rel="stylesheet" href="/layout-option-2.css?v=20260922"><link rel="stylesheet" href="/shop-seo.css?v=20260922b"><link rel="stylesheet" href="/analytics.css?v=20260917"><link rel="stylesheet" href="/header-cart.css?v=20260920">
...
function card(product, index = 5, heading = 'h3') {
  const name = copy.title(product), photo = images(product)[0], price = c.price(product);
  return `<article class="product-card product-card-compact" data-product-id="${esc(product.id)}"><a class="product-card-link" href="${esc(c.productPath(product))}">${photo ? `<img src="${esc(photo)}" alt="${esc(name)}" class="product-image" width="400" height="400" loading="${index < 2 ? 'eager' : 'lazy'}" decoding="async">` : '<div class="product-image product-image-placeholder">View product details</div>'}</a><p class="owner-meta">${esc(product.category)}</p><${heading}><a href="${esc(c.productPath(product))}">${esc(name)}</a></${heading}><p class="owner-meta">${esc(c.stockText(product))}</p><div class="price-wrap">${Number(product.discount) > 0 ? `<p class="old-price">${money(product.sell_price)}</p>` : ''}<p class="price">${money(price)} <span class="currency-label">NZD</span></p></div><div class="product-card-actions"><button class="btn btn-primary" type="button" data-add-product="${esc(product.id)}" ${c.purchasable(product) ? '' : 'disabled'}>${c.purchasable(product) ? 'Add to cart' : 'Out of stock'}</button><a class="btn btn-secondary" href="${esc(c.productPath(product))}" aria-label="View details for ${esc(name)}">View details</a></div></article>`;
}
...
function listingPage(catalogue, category, query) {
  ...
  let filtered = products.filter(p => (!category || category.matches(p)) && (!settings.hide_out_of_stock || c.purchasable(p)));
